...
def load_and_prepare_config(json_path):
    """加载配置文件，仅在内存中执行数据规范化与路径补全（绝不修改磁盘上的原 JSON 文件）。"""
    ...
    # 1. 内存中规范化同步源路径 (sync_source)
    sync_source = config_data.get(CONFIG_KEYS['SOURCES'], {})
    for url, targets in sync_source.items():
        sync_source[url] = _normalize_config_value(targets)

    # 2. 内存中规范化规则组 (groups) 的 inputs 与 outputs 映射
    for group in config_data.get(CONFIG_KEYS['GROUPS'], []):
        ...
        if CONFIG_KEYS['INPUTS'] not in group or not group[CONFIG_KEYS['INPUTS']]:
            ...
            group[CONFIG_KEYS['INPUTS']] = _normalize_config_value(group[CONFIG_KEYS['INPUTS']])
        ...
        for tool_key, config in ROUTING_MATRIX.items():
            ...
            # 内存防错与类型归一化处理
            if user_val is True:
                outputs[tool_key] = group_name_lower
            elif user_val is False:
                outputs[tool_key] = False
            elif user_val == '':
                is_passed = RuleFilter.evaluate(config, group_name_lower)
                outputs[tool_key] = group_name_lower if is_passed else False
