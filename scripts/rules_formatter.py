...
def generate_pac(matrix_data, output_dir):
    ...
    for g_name, g_rules in matrix_data.items():
        ...
        # 1. 合并 suffix 与 full 类型的域名
        combined_domains = set(g_rules.get('suffix', [])) | set(g_rules.get('full', []))
        
        # 2. 剔除所有以 .in-addr.arpa 和 .ip6.arpa 结尾的 DNS 底层反查域名
        filtered_domains = {
            domain for domain in combined_domains 
            if not domain.lower().endswith(('in-addr.arpa', 'ip6.arpa'))
        }
        
        direct_domains = sorted(filtered_domains)
        ...
        # 3. 核心内置 PAC 过滤逻辑 JS 脚本
        js_function = r"""var IS_IPV4_REGEXP = /^\d+\.\d+\.\d+\.\d+$/;
