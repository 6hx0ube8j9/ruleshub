...
async def fetch_all_remote_sources(config_data):
    ...
    print(f"[信息] 过滤去重完成，共收集到 {len(urls_to_fetch)} 个有效网络源链接")
...
def process_sources_in_memory(config_data, fetched_data):
    ...
    for url, targets in sync_source.items():
        ...
        if isinstance(targets, str):
            ...
            print(f"[警告] 同步源目标映射格式异常，跳过处理: {targets}")
            continue
            
        for target in target_list:
            target = target.strip().lower()
            if not target:
                print("[警告] 检测到空的规则目标标签，自动略过")
                continue
            filename = target if target.endswith('.txt') else f"{target}.txt"
            
            file_path = os.path.join(SOURCE_DIRECTORY, filename)
            # 统一生成绝对小写规范键，作为唯一基准
            norm_key = os.path.abspath(file_path).lower()
            
            if norm_key not in target_to_urls:
                target_to_urls[norm_key] = (file_path, [])
            target_to_urls[norm_key][1].append(url_clean)

    # === 通道 A: 网络同步源处理（聚合多源后统一合并清洗） ===
    for norm_key, (file_path, urls) in target_to_urls.items():
        filename = os.path.basename(file_path)
        
        local_raw = []
        if os.path.exists(file_path):
            print(f"[信息] 正在读取本地冷备快照文件: {filename}")
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    local_raw = f.read().splitlines()
            except Exception as e:
                print(f"[警告] 读取本地冷备快照失败 {filename}: {e}")
        else:
            print(f"[信息] 未发现本地冷备文件，将作为全新规则处理: {filename}")
            
        # 收集该目标对应的所有远程 URL 规则数据
        all_remote_lines = []
        ...
        for url in urls:
            ...
            if remote_lines is not None:
                ...
                all_remote_lines.extend(remote_lines)
            else:
                print(f"[警告] 远程拉取失败: {url}")

        # 内存清洗沙盒化判定
        if has_remote_data:
            print(f"[信息] 正在合并远程更新数据: {filename}")
            cleaned_rules = PROCESSOR_PIPELINE(local_raw, all_remote_lines)
        else:
            print(f"[警告] 远程拉取失败，正在回滚至本地快照: {filename}")
            cleaned_rules = PROCESSOR_PIPELINE(local_raw, [])
                
        # Key 统一存入规范化的绝对路径小写
        write_buffer[norm_key] = cleaned_rules

    # === 通道 B: 全量物理目录扫漏与独立清洗 ===
    if os.path.exists(SOURCE_DIRECTORY):
        processed_norm_keys = set(write_buffer.keys())
        ...
        for raw_filename in os.listdir(SOURCE_DIRECTORY):
            if raw_filename.lower().endswith('.txt'):
                ...
                norm_key = os.path.abspath(actual_path).lower()
                
                if norm_key in processed_norm_keys:
                    continue
...
def build_group_rules_pure_memory(group_config: dict, write_buffer: dict, fetched_data: dict = None) -> dict:
    ...
    if not inputs:
        ...
        buffer_key   = os.path.abspath(default_path).lower()
    ...
    for inp in inputs:
        ...
        # 命中网络直连 URL，执行纯内存直通解析，不落盘
        if inp_str.startswith(('http://', 'https://')):
            remote_lines = fetched_data.get(inp_str)
            if remote_lines:
                print(f"[信息] 正在为策略组注入实时网络规则: {group_name}")
        ...
        if os.path.isabs(inp_str):
            ...
            target_path = os.path.join(SOURCE_DIRECTORY, filename)
        
        buffer_key = os.path.abspath(target_path).lower()
    ...
    if isinstance(outputs, dict) and CONFIG_KEYS['QX_POLICY'] in outputs:
        ...
        if isinstance(custom_label, str) and custom_label.strip():
            if custom_label.lower() not in ['true', 'false']:
                print(f"[信息] 提取到自定义 QX 策略标签: {custom_label.strip()}")
...
def dispatch_to_matrix(group_name, group_config, rules, global_matrix):
    ...
    print(f"[信息] 开始分发策略组规则到多平台矩阵: {group_name}")
    ...
    print(f"[成功] 策略组 {group_name} 矩阵分发装配完成")
...
def build_routing_matrix_from_buffer(write_buffer, config_data, group_rules_cache, fetched_data):
    ...
    for group_config in groups:
        group_name = group_config.get('name')
        if not group_name:
            print("[警告] 发现缺失名称字段的策略组配置，自动忽略")
            continue
            
        print(f"[信息] 正在为策略组 {group_name} 解析内存规则并构建分流矩阵")
        ...
        if rules_in_memory is None:
            print(f"[警告] 策略组 {group_name} 解析出的规则为空，跳过分配")
            continue
...
def commit_write_buffer(write_buffer: dict):
    ...
    print(f"[信息] 开始执行规则源文件的原子级事务提交，待落盘总数: {len(write_buffer)}")
    if not os.path.exists(SOURCE_DIRECTORY):
        print(f"[信息] 创建规则存储源物理目录: {SOURCE_DIRECTORY}")
    ...
    for file_path, category_dict in write_buffer.items():
        ...
        if target_lower_name in lower_existing:
            ...
            if actual_name != base_name:
                try:
                    ...
                    print(f"[信息] 已成功清除冲突的大小写残留文件: {actual_name}")
                except Exception as e:
                    print(f"[警告] 删除大小写冲突文件失败 {actual_name}: {e}")
        ...
        try:
            ...
            print(f"[成功] 规则源原子落盘成功: {base_name}")
        except Exception as e:
            print(f"[错误] 文件原子事务保存失败 {base_name}: {e}")
            if os.path.exists(temp_path):
                os.remove(temp_path)

def safe_write_text(filepath, content):
    """通用工具：原子级写入文件（先写.tmp再替换），防止写入中断导致文件损坏。"""
    print(f"[信息] 启动文件原子级安全写入流程: {filepath}")
    temp_path = f"{filepath}.tmp"
    try:
        with open(temp_path, 'w', encoding='utf-8') as f:
            f.write(content)
        os.replace(temp_path, filepath)
        print(f"[成功] 文件原子级安全落盘成功: {filepath}")
        return True
    except Exception as e:
        print(f"[错误] 安全写入文件失败 {filepath}: {e}")
        if os.path.exists(temp_path):
...
def compile_mihomo_mrs(base_name, group_config, rules):
    ...
    if not os.path.exists(MIHOMO_COMPILER_CORE):
        print(f"[警告] 未找到 Mihomo 二进制编译工具，跳过编译: {MIHOMO_COMPILER_CORE}")
        return
    ...
    for tunnel_type, (sub_dir, formatter) in MIHOMO_ROUTING.items():
        ...
        if not content:
            print(f"[警告] 格式化后的 Mihomo 规则内容为空，放弃编译: {target_name}")
            continue
        ...
        if not safe_write_text(yaml_path, content):
            print(f"[错误] 写入 YAML 临时配置失败，终止 MRS 编译流程: {target_name}")
            continue
            
        try:
            print(f"[信息] 正在调用 Mihomo 二进制工具链编译 MRS 规则集: {target_name}")
            subprocess.run(
                [MIHOMO_COMPILER_CORE, 'convert-ruleset', sub_dir, 'yaml', yaml_path, mrs_out_path], 
                check=True, capture_output=True, text=True
            )
            print(f"[成功] 编译 MRS 完成: {target_name}.mrs ({sub_dir})")
        except subprocess.CalledProcessError as e:
            print(f"[警告] Mihomo 编译失败 {target_name}，退出码: {e.returncode}，错误信息: {e.stderr.strip()}")
...
def compile_singbox_srs(global_matrix, singbox_dir):
    ...
    if not os.path.exists(SINGBOX_COMPILER_CORE) or not global_matrix.get('singbox'): 
        print("[警告] 未找到 Sing-box 编译工具或目标矩阵为空，跳过 SRS 编译")
        return
    ...
    for g_name, raw_rules in global_matrix['singbox'].items():
        ...
        if not os.path.exists(sb_path): 
            print(f"[警告] 未找到编译所需的 Sing-box JSON 源文件: {sb_path}")
            continue

        try:
            print(f"[信息] 正在调用 Sing-box 二进制工具链编译 SRS 规则集: {g_name}")
            subprocess.run([SINGBOX_COMPILER_CORE, 'rule-set', 'compile', sb_path, '-o', sb_srs_path], check=True, capture_output=True, text=True)
            print(f"[成功] 编译 SRS 完成: {g_name}.srs")
        except subprocess.CalledProcessError as e:
            print(f"[警告] Sing-box 编译失败 {g_name}.json，退出码: {e.returncode}，错误信息: {e.stderr.strip()}")
...
async def async_main():
    ...
    try:
        ...
        print("\n[成功] 所有阶段均已成功执行，具备绝对的事务安全性！")
    except Exception as e:
        print(f"[严重错误] 阶段 4 提交或编译失败: {e}")
        sys.exit(1)

def main():
    """全局入口主函数与事件循环策略初始化。"""
    print("[信息] 初始化引擎并加载异步运行策略")
    if sys.platform == 'win32':
        asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
    asyncio.run(async_main())
    print("[成功] 全局流程生命周期完整安全结束")
